;(function (d) {
    function q(c, a, b) {
        var e = this, n = c.add(this), g = c.find(b.tabs), f = a.jquery ? a : c.children(a), k;
        g.length || (g = c.children());
        f.length || (f = c.parent().find(a));
        f.length || (f = d(a));
        d.extend(this, {
            click: function (a, h) {
                var f = g.eq(a), l = !c.data("tabs");
                "string" == typeof a && a.replace("#", "") && (f = g.filter('[href*\x3d"' + a.replace("#", "") + '"]'), a = Math.max(g.index(f), 0));
                if (b.rotate) {
                    var m = g.length - 1;
                    if (0 > a)return e.click(m, h);
                    if (a > m)return e.click(0, h)
                }
                if (!f.length) {
                    if (0 <= k)return e;
                    a = b.initialIndex;
                ...
                n.trigger(h, [a]);
                if (!h.isDefaultPrevented())return p[l ? b.initialEffect && b.effect || "default" : b.effect].call(e, a, function () {
                    k = a;
                    h.type = "onClick";
                    n.trigger(h, [a])
        ...
        f.find('a[href^\x3d"#"]').on("click.T", function (a) {
            e.click(d(this).attr("href"), a)
        });
        location.hash && "a" == b.tabs && c.find('[href\x3d"' + location.hash + '"]').length ? e.click(location.hash) : (0 === b.initialIndex || 0 < b.initialIndex) && e.click(b.initialIndex)
    }
